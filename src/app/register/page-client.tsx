...
import { Mail, Lock, Eye, EyeOff, User, Sparkles, Loader2, ArrowRight, ArrowLeft, Mic2 } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    // Create profile entry immediately after signup
    if (signUpData?.user) {
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center pt-24 pb-12 px-6 overflow-hidden bg-brand-bg select-none">
      ...
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden">
        <div className="absolute top-[20%] left-[-20%] w-[60%] h-[60%] bg-brand-primary/10 rounded-full blur-3xl" />
      ...
      <div className="w-full max-w-xl relative z-10">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-brand-muted-dark text-sm md:text-base">
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              ...
              >
                ...
                <motion.div 
                  ...
                  className="bg-brand-card border border-brand-border rounded-xl card-hover p-8 cursor-pointer transition-colors group relative overflow-hidden"
                >
                  ...
                  <h2 className="text-xl font-bold uppercase tracking-wider mb-2">Listener</h2>
                  <p className="text-sm text-brand-muted-dark leading-relaxed">
                ...
                >
                  ...
                  <h2 className="text-xl font-bold uppercase tracking-wider mb-2">Artist</h2>
                  <p className="text-sm text-brand-muted-dark leading-relaxed">
            ...
            {step === 2 && (
              <motion.div
                ...
                exit={{ opacity: 0, x: 20 }}
                transition={{ duration: 0.3 }}
                className="bg-brand-card border border-brand-border rounded-xl p-8 md:p-10 relative"
              >
                <button 
                  onClick={() => setStep(1)}
                  className="absolute top-6 left-6 text-brand-muted-dark hover:text-white flex items-center text-xs font-bold uppercase tracking-widest transition-colors"
                >
                ...
                <div className="mt-8 mb-8 space-y-3">
                  ...
                  <button 
                    ...
                    className="w-full relative flex items-center justify-center gap-3 py-3.5 px-4 rounded-lg bg-white text-black font-bold text-sm tracking-wide hover:bg-neutral-100 transition-colors"
                  >
                ...
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <span className="text-xs font-bold tracking-widest text-brand-muted-dark uppercase">Or create via email</span>
                  <div className="h-px bg-white/10 flex-1" />
                </div>

                <form onSubmit={handleRegister} className="space-y-4">
                  <div>
                    <label htmlFor="register-name" className="sr-only">{role === 'artist' ? "Stage Name" : "Full Name"}</label>
                    <div className="relative group">
                      ...
                      <input
                        ...
                        className="w-full bg-brand-bg border border-brand-border rounded-lg px-4 py-3 pl-12 text-white placeholder:text-brand-muted focus:outline-none focus:border-brand-primary/50 focus:ring-1 focus:ring-brand-primary/50 transition-all"
                  ...
                  <div>
                    ...
                    <div className="relative group">
                      ...
                      <input
                        ...
                        className="w-full bg-brand-bg border border-brand-border rounded-lg px-4 py-3 pl-12 pr-12 text-white placeholder:text-brand-muted focus:outline-none focus:border-brand-primary/50 focus:ring-1 focus:ring-brand-primary/50 transition-all"
                        placeholder="Create Password"
                      />
                      <button
                        type="button"
                        onClick={() => setShowPassword(!showPassword)}
                        className="absolute inset-y-0 right-0 pr-4 flex items-center text-brand-muted-dark hover:text-white transition-colors"
                      >
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className={`w-full relative flex items-center justify-center gap-2 py-3.5 px-4 rounded-lg text-black font-bold uppercase tracking-wider text-sm hover:opacity-90 transition-opacity mt-4 disabled:opacity-50 ${
                      role === 'artist' 
                ...
                <p className="text-center text-sm text-brand-muted-dark mt-6">
