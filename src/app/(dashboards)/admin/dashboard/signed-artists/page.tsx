...
import { Plus, Pencil, Trash2, X, Save, Loader2, Mic2, CheckCircle2, Edit2 } from "lucide-react";
import Link from "next/link";
import PageMeta from "@/components/seo/PageMeta";

const defaultSignedArtists = [
  {
    id: "sahir-alam",
    name: "SAHIR ALAM",
...
export default function ManageSignedArtistsPage() {
  ...
  const fetchArtists = async () => {
    ...
    if (data && data.value) {
      ...
      await supabase
        .from("settings")
        .upsert({ key: "signed_artists_data", value: defaultSignedArtists });
    }
    setLoading(false);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      .from("settings")
      .upsert({ key: "signed_artists_data", value: updatedList });

    if (!error) {
      setArtists(updatedList);
      setIsModalOpen(false);
      alert("Signed artist saved successfully!");
    } else {
      alert("Error saving signed artist: " + error.message);
    }
  };

  const handleDelete = async (artistId: string) => {
    const confirmDelete = window.confirm("Are you sure you want to permanently remove this signed artist?");
    ...
    const { error } = await supabase
      .from("settings")
      .upsert({ key: "signed_artists_data", value: filtered });

    if (!error) {
      setArtists(filtered);
      alert("Signed artist permanently deleted!");
    } else {
      alert("Error deleting signed artist: " + error.message);
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto pt-8 px-4 md:px-0">
      <PageMeta title="Manage Signed Artists" description="Admin — manage ESSKAYTONALITY signed artist roster." noIndex />
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold uppercase tracking-tighter mb-1">
              Signed <span className="text-amber-500">Artists</span>
            </h1>
            <p className="text-brand-muted-dark text-sm">Manage the official ESSKAYTONALITY roster — signed artists with exclusive content.</p>
          </div>
        </div>

        <button
          onClick={openAddModal}
          className="flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg px-5 py-2.5 font-medium uppercase tracking-wider transition-all text-sm w-fit"
        >
          <Plus className="w-4 h-4" /> Add Signed Artist
        </button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                <tr>
                  <td colSpan={5} className="p-12 text-center text-brand-muted-dark">Loading signed artists...</td>
                </tr>
              ) : artists.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-12 text-center text-brand-muted-dark">No signed artists found.</td>
                </tr>
              ) : (
                artists.map((artist) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 overflow-y-auto">
          <div className="relative w-full max-w-4xl bg-brand-card border border-brand-border rounded-xl p-6 md:p-8 max-h-[90vh] overflow-y-auto">
            ...
            <h2 className="text-2xl font-bold uppercase tracking-tight mb-8">
              {currentEditingArtist ? "Edit Signed Artist" : "Add Signed Artist"}
            </h2>
            ...
            <form onSubmit={handleSave} className="space-y-8 text-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-brand-muted-dark mb-2">Artist Name *</label>
                  <input
                    type="text"
                    required
                    value={formData.name}
                    ...
                    className="w-full bg-brand-bg border border-brand-border rounded-lg px-4 py-3 text-white focus:outline-none focus:border-amber-500"
                  />
                </div>
                <div>
              ...
              <div className="pt-6 border-t border-brand-border space-y-4">
                <h3 className="text-sm font-bold uppercase tracking-widest text-amber-500">Streaming Platforms Links</h3>
              ...
              <div className="pt-6 border-t border-brand-border space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-bold uppercase tracking-widest text-amber-500">Popular Tracks (Audio Player)</h3>
                  <button
                    type="button"
                    onClick={addFormTrack}
                    className="flex items-center gap-1 text-xs font-bold uppercase tracking-wider text-amber-500 hover:text-white transition-colors"
                  >
              ...
              <div className="pt-6 border-t border-brand-border space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-bold uppercase tracking-widest text-amber-500">Discography Albums (Music Videos)</h3>
                  <button
                    type="button"
              ...
              <div className="pt-8 border-t border-brand-border flex justify-end gap-4">
                ...
                <button
                  type="submit"
                  className="bg-amber-500 hover:bg-amber-600 text-white rounded-lg px-5 py-2.5 font-medium uppercase tracking-wider transition-all"
                >
                  Save Signed Artist
                </button>
              </div>
