...
import { Mic2, ArrowLeft, Trash2, Edit2, Plus, X, CheckCircle2 } from "lucide-react";
    ...
    <div className="max-w-6xl mx-auto pt-8 px-4 md:px-0">
      ...
      <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-brand-muted text-sm">Create, edit, and delete dynamic verified artist accounts on the platform.</p>
          </div>
        </div>

        <button 
          onClick={openAddModal}
          className="flex items-center gap-2 bg-brand-primary hover:bg-brand-accent text-white px-6 py-3 rounded-full font-bold uppercase tracking-wider transition-all transform hover:scale-105 text-sm w-fit"
        >
          ...
          <table className="w-full text-left">
            <thead className="bg-white/5 border-b border-white/10">
              <tr>
                ...
                <th className="p-6 text-xs font-bold tracking-widest text-brand-muted uppercase">Genre</th>
                <th className="p-6 text-xs font-bold tracking-widest text-brand-muted uppercase">Listeners</th>
              ...
              {loading ? (
                ...
                artists.map((artist) => (
                  <tr key={artist.id} className="hover:bg-white/5 transition-colors group">
                    <td className="p-6 text-xs text-brand-muted font-mono uppercase tracking-wider">{artist.id}</td>
                    ...
                    <td className="p-6 text-sm text-brand-muted">{artist.genre || "N/A"}</td>
                    <td className="p-6 text-sm text-brand-muted">{artist.listeners || "0"}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md overflow-y-auto">
          <div className="relative w-full max-w-4xl bg-brand-surface border border-white/10 rounded-3xl p-6 md:p-8 max-h-[90vh] overflow-y-auto shadow-2xl glass">
            ...
            <form onSubmit={handleSave} className="space-y-8 text-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-brand-muted mb-2">Stage / Rockstar Name *</label>
                  <input 
                    type="text"
                    required
                    value={formData.name}
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-brand-muted mb-2">Genre / Style Tagline</label>
                  <input 
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-brand-muted mb-2">Monthly Listeners Count</label>
                  <input 
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-brand-muted mb-2">Profile Avatar Image URL</label>
                  <input 
                    type="text"
                ...
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold uppercase tracking-wider text-brand-muted mb-2">Hero Profile Banner Image URL</label>
                  <input 
                    type="text"
                ...
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold uppercase tracking-wider text-brand-muted mb-2">Biography (Bio)</label>
                  <textarea 
                    rows={4}
                    value={formData.bio}
              ...
              <div className="pt-6 border-t border-white/5 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div>
                    <label className="block text-xs font-semibold text-brand-muted mb-2">Spotify Link</label>
                    <input 
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-brand-muted mb-2">Apple Music Link</label>
                    <input 
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-brand-muted mb-2">Deezer Link</label>
                    <input 
                      type="text"
              ...
              <div className="pt-6 border-t border-white/5 space-y-4">
                ...
                {formData.popular.map((track, idx) => (
                  <div key={idx} className="flex flex-col md:flex-row gap-4 items-center bg-black/20 p-4 rounded-2xl border border-white/5 relative">
                    <button 
                      type="button" 
                    ...
                    <div className="w-full">
                      <label className="block text-[10px] uppercase font-bold text-brand-muted mb-1">Track Name</label>
                      <input 
                        type="text"
                        value={track.title}
                    ...
                    <div className="w-full md:w-32">
                      <label className="block text-[10px] uppercase font-bold text-brand-muted mb-1">Streams Count</label>
                      <input 
                        type="text"
                        value={track.streams}
                    ...
                    <div className="w-full md:w-24">
                      <label className="block text-[10px] uppercase font-bold text-brand-muted mb-1">Duration</label>
                      <input 
                        type="text"
                        value={track.time}
                    ...
                    <div className="w-full">
                      <label className="block text-[10px] uppercase font-bold text-brand-muted mb-1">YouTube Audio Video ID</label>
                      <input 
                        type="text"
              ...
              <div className="pt-6 border-t border-white/5 space-y-4">
                ...
                {formData.albums.map((album, idx) => (
                  <div key={idx} className="flex flex-col md:flex-row gap-4 items-center bg-black/20 p-4 rounded-2xl border border-white/5 relative">
                    ...
                    <div className="w-full">
                      <label className="block text-[10px] uppercase font-bold text-brand-muted mb-1">Album Title</label>
                      <input 
                        type="text"
                        value={album.title}
                    ...
                    <div className="w-full md:w-32">
                      <label className="block text-[10px] uppercase font-bold text-brand-muted mb-1">Release Year</label>
                      <input 
                        type="text"
                        value={album.year}
                    ...
                    <div className="w-full">
                      <label className="block text-[10px] uppercase font-bold text-brand-muted mb-1">Cover Image URL</label>
                      <input 
                        type="text"
                        value={album.cover}
                    ...
                    <div className="w-full md:w-48">
                      <label className="block text-[10px] uppercase font-bold text-brand-muted mb-1">YouTube Video ID</label>
                      <input 
                        type="text"
              ...
              <div className="pt-8 border-t border-white/10 flex justify-end gap-4">
                <button 
                  type="button" 
                  onClick={() => setIsModalOpen(false)}
                  className="px-6 py-3 border border-white/20 hover:border-white/40 rounded-full font-bold uppercase tracking-wider transition-colors text-white"
                >
                  Cancel
                </button>
                <button 
                  type="submit" 
                  className="px-8 py-3 bg-brand-primary hover:bg-brand-accent text-white rounded-full font-bold uppercase tracking-wider transition-all transform hover:scale-105"
                >
                  Save Rockstar
                </button>
              </div>
