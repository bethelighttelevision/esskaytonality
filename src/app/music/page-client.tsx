...
export default function MusicPage() {
  ...
  useEffect(() => {
    const fetchReleases = async () => {
      const { data } = await supabase.from("releases").select("*").eq("status", "approved").order("created_at", { ascending: false });
    ...
    <div className="pt-32 pb-40 container mx-auto px-6 md:px-12 min-h-screen relative overflow-hidden bg-brand-bg select-none">
      ...
      <div className="mb-16 text-center max-w-3xl mx-auto">
        ...
        <motion.p 
          ...
          className="text-brand-muted-dark text-sm md:text-base max-w-xl mx-auto"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start relative z-10">
        ...
        >
          <div className="flex items-center justify-between border-b border-white/5 pb-4 mb-4">
            ...
            <div className="text-xs text-brand-muted-dark uppercase font-bold tracking-widest flex items-center gap-1">
                ...
                <div 
                  ...
                  onClick={() => handleSelectTrack(idx)}
                   className={`group relative flex items-center justify-between p-4 rounded-2xl border transition-all duration-300 cursor-pointer card-hover ${
                    isSelected 
                      ? "bg-brand-surface border-brand-primary/30" 
                  ...
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="w-8 h-8 flex items-center justify-center shrink-0">
                      {isSelected && isPlaying ? (
                        ...
                        <span className="text-sm font-bold text-brand-muted-dark group-hover:text-white transition-colors">
                          {idx + 1}
                        </span>
                      )}
                    </div>
      ...
      <div className="mt-32 pt-16 border-t border-white/5 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
          <div>
            ...
            <p className="text-brand-muted-dark">Explore our full albums, EPs, and curated collections.</p>
          </div>
          
          {/* Filters & Sorting */}
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2 bg-brand-primary rounded-lg px-5 py-2.5">
              <Filter className="w-4 h-4 text-brand-muted-dark" />
              <select 
                value={activeGenre}
                onChange={(e) => setActiveGenre(e.target.value)}
                className="bg-brand-bg border border-brand-border rounded-lg px-4 py-3 text-sm text-white font-bold uppercase tracking-wider outline-none cursor-pointer"
              >
            ...
            <div className="flex items-center gap-2 bg-brand-primary rounded-lg px-5 py-2.5">
              <ArrowUpDown className="w-4 h-4 text-brand-muted-dark" />
              <select 
                value={sortOrder}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {filteredAndSortedAlbums.map((album) => (
            <motion.div 
              ...
              className="bg-brand-card border border-brand-border rounded-xl overflow-hidden transition-all duration-300 cursor-pointer group card-hover"
            >
              <div className="relative aspect-square overflow-hidden border-b border-white/5">
                <img loading="lazy" src={album.cover} alt={album.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="text-xs font-bold text-brand-muted-dark">{new Date(album.releaseDate).getFullYear()}</span>
                </div>
                ...
                <p className="text-sm text-brand-muted-dark truncate mt-1">{album.artist}</p>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        {selectedAlbum && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-5xl max-h-[90vh] bg-brand-surface rounded-xl border border-white/10 overflow-hidden flex flex-col md:flex-row relative"
            ...
            >
              ...
              <div className="w-full md:w-2/5 p-8 border-b md:border-b-0 md:border-r border-white/5 bg-black/20 flex flex-col items-center">
                <div className="w-full aspect-square rounded-2xl overflow-hidden border border-white/10 shadow-2xl mb-8 relative group">
                  <img loading="lazy" src={selectedAlbum.cover} alt={selectedAlbum.title} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <button 
                ...
                <h2 className="text-3xl font-bold uppercase tracking-tighter text-center mb-2">{selectedAlbum.title}</h2>
                <p className="text-brand-muted-dark text-center mb-6">{selectedAlbum.artist}</p>
                ...
                <div className="flex gap-4 w-full">
                  ...
                  <button className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center hover:bg-white/5 transition-colors text-brand-muted-dark hover:text-white">
              ...
              <div className="w-full md:w-3/5 flex flex-col h-[50vh] md:h-auto">
                <div className="flex border-b border-white/5 p-4 gap-2">
                  <button onClick={() => setActiveTab("tracks")} className={`flex-1 py-3 text-xs font-bold uppercase tracking-widest rounded-lg flex items-center justify-center gap-2 transition-all ${activeTab === "tracks" ? "bg-white/10 text-white" : "text-brand-muted-dark hover:bg-white/5"}`}><ListMusic className="w-4 h-4"/> Tracklist</button>
                  <button onClick={() => setActiveTab("credits")} className={`flex-1 py-3 text-xs font-bold uppercase tracking-widest rounded-lg flex items-center justify-center gap-2 transition-all ${activeTab === "credits" ? "bg-white/10 text-white" : "text-brand-muted-dark hover:bg-white/5"}`}><Users className="w-4 h-4"/> Credits</button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                    ...
                    <div className="space-y-2">
                      {selectedAlbum.tracks.map((track: any, idx: number) => (
                        <div 
                          ...
                          className="flex items-center justify-between p-4 rounded-xl hover:bg-white/5 transition-colors group cursor-pointer border border-transparent hover:border-white/5 card-hover"
                        >
                          <div className="flex items-center gap-4">
                            <span className="text-brand-muted-dark font-bold text-sm w-4">{idx + 1}</span>
                          ...
                          <span className="text-xs font-bold text-brand-muted-dark">{track.duration}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {activeTab === "credits" && (
                    <div className="space-y-6">
                      <div>
                        <h3 className="text-xs font-bold uppercase tracking-widest text-brand-accent mb-2">Album Info</h3>
                        <p className="text-sm text-brand-muted-dark leading-relaxed">{selectedAlbum.description}</p>
                      </div>
                      <div>
                        <h3 className="text-xs font-bold uppercase tracking-widest text-brand-accent mb-2">Production Credits</h3>
                        <p className="text-sm text-brand-muted-dark leading-relaxed">{selectedAlbum.credits}</p>
                      </div>
                      <div>
                        <h3 className="text-xs font-bold uppercase tracking-widest text-brand-accent mb-2">Release Date</h3>
                        <p className="text-sm text-brand-muted-dark leading-relaxed">{new Date(selectedAlbum.releaseDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                      </div>
                    </div>
                  )}
                </div>
        ...
        <div className="py-3 px-4 md:py-5 md:px-12 flex items-center justify-between gap-4">
          ...
          <div className="hidden md:flex items-center gap-6 justify-center">
            ...
            <button 
              ...
              className="p-2 text-brand-muted-dark hover:text-white transition-colors"
            ...
            <button 
              onClick={togglePlay}
              className="w-12 h-12 rounded-full bg-brand-primary hover:bg-white text-black flex items-center justify-center transition-all duration-300"
          ...
          <div className="flex md:hidden items-center gap-2.5 shrink-0">
            ...
            <button 
              onClick={togglePlay}
              className="w-9 h-9 rounded-full bg-brand-primary text-black flex items-center justify-center"
              title={isPlaying ? "Pause" : "Play"}
            >
              {isPlaying ? (
          ...
          <div className="hidden md:flex items-center gap-3 w-1/3 justify-end shrink-0">
            <button 
              onClick={toggleMute}
              className="text-brand-muted-dark hover:text-white transition-colors p-2"
...
const NowPlayingShowcase = memo(({ isPlaying, currentTrack }: { isPlaying: boolean; currentTrack: any }) => {
  return (
    <div className="bg-brand-card border border-brand-border p-8 rounded-xl relative overflow-hidden flex flex-col items-center">
      ...
      <h2 className="text-xs font-bold uppercase tracking-widest text-brand-muted-dark mb-8 flex items-center gap-2">
        <Sparkles className="w-4 h-4 text-brand-accent" /> Now Playing Showcase
      </h2>
      ...
      <div className="relative w-64 h-64 flex items-center justify-center mb-8 mt-4 group">
        ...
        >
          <motion.div 
            ...
            transition={
              isPlaying 
                ? { rotate: { repeat: Infinity, duration: 12, ease: "linear" as const } } 
                : { duration: 0.5 }
            }
        ...
        <motion.div
          ...
          className="relative z-10 w-56 h-56 rounded-2xl overflow-hidden border border-white/10 bg-brand-surface shrink-0"
        >
          <img loading="lazy" src={currentTrack.image} alt={currentTrack.title} className="w-full h-full object-cover" />
      ...
      <div className="text-center w-full px-4 mb-8">
        ...
        <p className="text-sm text-brand-muted-dark truncate mt-1">
          {currentTrack.artist}
        </p>
      </div>
