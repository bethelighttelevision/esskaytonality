    ...
    <div className="w-full pt-24 pb-12 min-h-screen">
      ...
      <section className="container mx-auto px-6 md:px-12">
        ...
        >
          {newsItems.filter(item => !item.featured).map((news) => (
            <motion.div 
              key={news.id}
              variants={item}
              className="glass rounded-xl overflow-hidden group border border-white/5 hover:border-brand-primary/30 transition-all duration-300 flex flex-col h-full"
            >
              ...
              <div className="p-8 flex flex-col flex-grow">
                <div className="text-xs text-brand-muted mb-3 flex items-center gap-2">
        ...
        <div className="mt-16 flex justify-center">
          <button className="px-8 py-4 rounded-full border border-white/20 hover:border-brand-accent hover:bg-brand-accent/10 transition-all duration-300 text-sm font-bold uppercase tracking-widest flex items-center gap-3">
            Load More Content
          </button>
        </div>
      </section>
