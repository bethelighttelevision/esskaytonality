...
const defaultArtists: Record<string, any> = {
  "sahir-alam": {
    name: "SAHIR ALAM",
...
export default function ArtistDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchArtistDetails = async () => {
      ...
      if (found) {
        ...
        // Fallback to static mock if not saved yet
  ...
  // Monitor progress from YouTube Background Player
  useEffect(() => {
    let timer: any = null;
  ...
  // Robust failsafe sharing mechanism
  const handleShare = () => {
  ...
  return (
    <div className="w-full min-h-screen pb-24">
      {/* Background Hidden YouTube Player Container Mount */}
              ...
              {artist.popular.map((track: any, idx: number) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-bold text-white group-hover:text-brand-primary transition-colors">{track.title}</h4>
              ...
              {artist.albums.map((album: any, idx: number) => (
                ...
                >
                  <div className="aspect-square rounded-xl overflow-hidden mb-4 relative">
                    ...
                    {/* Hover Play Button Overlay with sound waves animation */}
                  ...
                  <h4 className="font-bold text-white truncate group-hover:text-brand-primary transition-colors">{album.title}</h4>
          ...
          <section className="glass p-8 rounded-2xl">
            ...
            <div className="flex flex-col gap-4 text-sm font-semibold text-white">
              <div className="flex justify-between items-center py-2 border-b border-white/10">
                ...
                <span>{isFollowing ? "24,592,105" : "24,592,104"}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-white/10">
      ...
      {/* Cinematic Video Overlay Iframe player popup */}
      <AnimatePresence>
        {activeVideo && (
          <motion.div 
            initial={{ opacity: 0 }}
